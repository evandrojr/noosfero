...
class CkeditorArticleTest < ActiveSupport::TestCase
  ...
  should 'be an article' do
    assert_subclass TextArticle, CkeditorContentPlugin::CkeditorArticle
  end

  should 'define description' do
    assert_kind_of String, CkeditorContentPlugin::CkeditorArticle.description
  end

  should 'define short description' do
    assert_kind_of String, CkeditorContentPlugin::CkeditorArticle.short_description
  end

  should 'not sanitize target attribute' do
    article = create(CkeditorContentPlugin::CkeditorArticle, :name => 'open link in new window', :body => "open <a href='www.invalid.com' target='_blank'>link</a> in new window", :profile => profile)
    assert_tag_in_string article.body, :tag => 'a', :attributes => {:target => '_blank'}
  end

  should 'not translate & to amp; over times' do
    article = create(CkeditorContentPlugin::CkeditorArticle, :name => 'link', :body => "<a href='www.invalid.com?param1=value&param2=value'>link</a>", :profile => profile)
    assert article.save
  ...
  should 'not escape comments from tiny mce article body' do
    article = create(CkeditorContentPlugin::CkeditorArticle, :profile => profile, :name => 'article', :abstract => 'abstract', :body => "the <!-- comment --> article ...")
    assert_equal "the <!-- comment --> article ...", article.body
  end

  should 'convert entities characters to UTF-8 instead of ISO-8859-1' do
    article = create(CkeditorContentPlugin::CkeditorArticle, :profile => profile, :name => 'teste ' + Time.now.to_s, :body => '<a title="inform&#225;tica">link</a>')
  ...
  should 'remove iframe if it is not from a trusted site' do
    article = create(CkeditorContentPlugin::CkeditorArticle, :profile => profile, :name => 'article', :abstract => 'abstract', :body => '<iframe src="http://anything/videos.ogg"></iframe>')
  ...
  should 'not mess with <iframe and </iframe if it is from itheora by default' do
    ...
    article = create(CkeditorContentPlugin::CkeditorArticle, :profile => profile, :name => 'article', :abstract => 'abstract', :body => "<iframe src='http://itheora.org/demo/index.php?v=example.ogv'></iframe>")
  ...
  should 'allow iframe if it is from stream.softwarelivre.org by default' do
    ...
    article = create(CkeditorContentPlugin::CkeditorArticle, :profile => profile, :name => 'article', :abstract => 'abstract', :body => "<iframe src='http://stream.softwarelivre.org/fisl10/sites/default/files/videos.ogg'></iframe>")
  ...
  should 'allow iframe if it is from tv.softwarelivre.org by default' do
    ...
    article = create(CkeditorContentPlugin::CkeditorArticle, :profile => profile, :name => 'article', :abstract => 'abstract', :body => "<iframe id='player-base' src='http://tv.softwarelivre.org/embed/1170' width='482' height='406' align='right' frameborder='0' scrolling='no'></iframe>")
  ...
  should 'allow iframe if it is from a trusted site' do
    ...
    article = create(CkeditorContentPlugin::CkeditorArticle, :profile => profile, :name => 'article', :abstract => 'abstract', :body => "<iframe src='http://avideosite.com/videos.ogg'></iframe>")
  ...
  should 'remove only the iframe from untrusted site' do
    article = create(CkeditorContentPlugin::CkeditorArticle, :profile => profile, :name => 'article', :abstract => 'abstract', :body => "<iframe src='http://stream.softwarelivre.org/videos.ogg'></iframe><iframe src='http://untrusted_site.com/videos.ogg'></iframe>")
  ...
  should 'not sanitize html comments' do
    article = CkeditorContentPlugin::CkeditorArticle.new
  ...
  should 'not allow XSS on name' do
    article = create(CkeditorContentPlugin::CkeditorArticle, :name => 'title with <script>alert("xss")</script>', :profile => profile)
    assert_no_match /script/, article.name
  end

  should 'not allow XSS on abstract' do
    article = create(CkeditorContentPlugin::CkeditorArticle, :name => "test 123", :abstract => 'abstract with <script>alert("xss")</script>', :profile => profile)
    assert_no_match /script/, article.abstract
  end

  should 'notifiable be true' do
    a = fast_create(CkeditorContentPlugin::CkeditorArticle)
    assert a.notifiable?
  end

  should 'notify activity on create' do
    ActionTracker::Record.delete_all
    create CkeditorContentPlugin::CkeditorArticle, :name => 'test', :profile_id => fast_create(Profile).id, :published => true
  ...
  should 'not group trackers activity of article\'s creation' do
    ...
    create CkeditorContentPlugin::CkeditorArticle, :name => 'bar', :profile_id => profile.id, :published => true
    create CkeditorContentPlugin::CkeditorArticle, :name => 'another bar', :profile_id => profile.id, :published => true
    assert_equal 2, ActionTracker::Record.count
    create CkeditorContentPlugin::CkeditorArticle, :name => 'another bar', :profile_id => fast_create(Profile).id, :published => true
    assert_equal 3, ActionTracker::Record.count
  end

  should 'not create trackers activity when updating articles' do
    ActionTracker::Record.delete_all
    a1 = create CkeditorContentPlugin::CkeditorArticle, :name => 'bar', :profile_id => fast_create(Profile).id, :published => true
    a2 = create CkeditorContentPlugin::CkeditorArticle, :name => 'another bar', :profile_id => fast_create(Profile).id, :published => true
  ...
  should "the tracker action target be defined as the article on articles'creation in communities" do
    ...
    article = create CkeditorContentPlugin::CkeditorArticle, :name => 'test', :profile_id => community.id
  ...
  should "the tracker action target be defined as the article on articles'creation in profile" do
    ActionTracker::Record.delete_all
    person = Person.first
    article = create CkeditorContentPlugin::CkeditorArticle, :name => 'test', :profile_id => person.id
    assert_equal article, ActionTracker::Record.last.target
  end

  should 'not notify activity if the article is not advertise' do
    ActionTracker::Record.delete_all
    a = create CkeditorContentPlugin::CkeditorArticle, :name => 'bar', :profile_id => fast_create(Profile).id, :published => true, :advertise => false
  ...
  should "have defined the is_trackable method defined" do
    assert CkeditorContentPlugin::CkeditorArticle.method_defined?(:is_trackable?)
  end

  should "the common trackable conditions return the correct value" do
    a =  build(CkeditorContentPlugin::CkeditorArticle, :profile => profile)
  ...
  should 'ck editor is enabled' do
    assert CkeditorContentPlugin::CkeditorArticle.new.ck_editor?
  end

  should 'not sanitize html5 audio tag on body' do
    article = CkeditorContentPlugin::CkeditorArticle.create!(:name => 'html5 audio', :body => "Audio: <audio controls='controls'><source src='http://example.ogg' type='audio/ogg' />Audio not playing?.</audio>", :profile => profile)
  ...
  should 'not sanitize html5 video tag on body' do
    article = CkeditorContentPlugin::CkeditorArticle.create!(:name => 'html5 video', :body => "Video: <video controls='controls' autoplay='autoplay'><source src='http://example.ogv' type='video/ogg' />Video not playing?</video>", :profile => profile)
  ...
  should 'not sanitize colspan and rowspan attributes' do
    article = CkeditorContentPlugin::CkeditorArticle.create!(:name => 'table with colspan and rowspan',
