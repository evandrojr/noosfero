function getNextParagraphId() {
  max = -1;
  paragraphs = jQuery('#article_body_ifr').contents().find('.article_comments');
  paragraphs.each(function(key, value) {
    value = jQuery(value).attr('data-macro-paragraph_id');
...
function makeCommentable() {
  ...
  if(!hasTag) {
    ...
    tags.wrapAll('<div class=\"macro article_comments\" data-macro=\"comment_paragraph_plugin/allow_comment\" data-macro-paragraph_id=\"'+getNextParagraphId()+'\"/>');
